...
export default function JobDetailPage() {
  ...
  const fetchJob = async () => {
    try {
      ...
      const found = data.listings?.find((l: JobDetail) => l.slug === slug)
      setJob(found || null)
    } catch {
  ...
  const fetchComments = async () => {
    try {
      // We need the job ID to fetch comments, but we have the slug
      // Comments will load once we have the job data
    } catch {
      // handled below
    }
  }
  ...
  if (!job) {
    return (
      <div className="min-h-dvh bg-white">
        <div className="mx-auto max-w-5xl px-5 sm:px-6 py-20 text-center">
          <h1 className="text-2xl font-extrabold text-slate-900 mb-4">Job not found</h1>
