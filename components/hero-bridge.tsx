...
export function HeroBridge() {
  return (
    ...
    >
      ...
      {/* Hero Video - Takes up available space but constrained */}
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
        className="w-[calc(100%+3rem)] -mx-6 sm:w-[calc(100%+4rem)] sm:-mx-8 md:w-full md:mx-0 max-w-none md:max-w-4xl lg:max-w-6xl shrink-0"
      >
        <video
          src="https://devsa-assets.s3.us-east-2.amazonaws.com/Seamless_Clapper_Loop.mp4"
          autoPlay
          loop
          muted
          playsInline
          className="w-full h-auto md:h-80 object-contain"
        />
      </motion.div>

      {/* Text content - directly below image */}
      <div className="w-full max-w-3xl text-center shrink-0 px-2">
